...
export interface ChatMessage {
  ...
  rolTutor?: string;
  rutaRetrieval?: 'PGVECTOR' | 'MEMORIA';
  nroChunks?: number;
  paginasCubiertas?: number;
  embeddingsDisponibles?: boolean;
  timestamp: Date;
  showSources?: boolean;
}
...
export class App implements OnInit {
  ...
  isIndexedWithoutVectors(doc: RagDocumentInfo): boolean {
    return doc.indexedWithVectors === false;
  }

  docCobertura(doc: RagDocumentInfo): string {
    const conTexto = doc.pageCountConTexto ?? doc.pageCount;
    if (!doc.pageCount) return '';
    return `${conTexto}/${doc.pageCount} págs. con texto`;
  }
    ...
    this.ragService.chatWithTutor({
      ...
      next: (resp) => {
        ...
        const tutorMsg: ChatMessage = {
          ...
          rutaRetrieval: resp.rutaRetrieval,
          nroChunks: resp.nroChunks,
          paginasCubiertas: resp.paginasCubiertas,
          embeddingsDisponibles: resp.embeddingsDisponibles,
          timestamp: new Date(),
          showSources: false
        };
