...
public class InMemoryRagVectorStore {
    ...
    public List<DocumentChunk> searchTopK(String docId, String query, int k) {
        ...
        for (DocumentChunk chunk : chunks) {
            ...
            // Normalización por largo: los chunks largos acumulan más términos en común
            // y dominarían el ranking; se pondera para equilibrar páginas de distinto tamaño.
            int wordCount = countWords(chunk.getContent());
            double lengthNorm = 1.0 + Math.log1p(wordCount);
            double finalScore = (dotProduct + (commonTerms * 0.08)) / lengthNorm;
        ...
        // Selección top-K con cobertura de páginas: un máximo de chunks por página evita
        // que una sola página (p.ej. la portada) acapare todo el contexto.
        List<DocumentChunk> results = new ArrayList<>();
        Map<Integer, Integer> usedPerPage = new HashMap<>();
        int maxPerPage = Math.max(1, k / 2);

        for (ScoredChunk sc : scoredChunks) {
            if (results.size() >= k) break;
            int page = sc.chunk().getPageNumber();
            int used = usedPerPage.getOrDefault(page, 0);
            if (used >= maxPerPage) continue;
            usedPerPage.merge(page, 1, Integer::sum);
            results.add(sc.chunk());
        }

        // Pasada de relleno: si quedan cupos, completar con los siguientes mejores sin
        // limitación de página (solo para no desperdiciar contexto disponible).
        if (results.size() < k) {
            for (ScoredChunk sc : scoredChunks) {
                if (results.size() >= k) break;
                if (results.contains(sc.chunk())) continue;
                results.add(sc.chunk());
            }
        }
