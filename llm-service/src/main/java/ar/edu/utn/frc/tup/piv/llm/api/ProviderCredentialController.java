...
public class ProviderCredentialController {
  private final ProviderCredentialRepository repository; private final EncryptedSecretService crypto; private final ProviderLlmGateway gateway; private final GoldenSetAuthorization authorization; private final EvaluatorModelEvents events; private final GatewayExecutor executor;
  @org.springframework.beans.factory.annotation.Autowired
  public ProviderCredentialController(ProviderCredentialRepository repository, EncryptedSecretService crypto, ProviderLlmGateway gateway, GoldenSetAuthorization authorization, EvaluatorModelEvents events, GatewayExecutor executor) { this.repository=repository; this.crypto=crypto; this.gateway=gateway; this.authorization=authorization; this.events=events; this.executor=executor; }
  public ProviderCredentialController(ProviderCredentialRepository repository, EncryptedSecretService crypto, ProviderLlmGateway gateway, GoldenSetAuthorization authorization, EvaluatorModelEvents events) { this(repository,crypto,gateway,authorization,events,GatewayExecutor.disabled()); }
  /** Las pruebas de admin también pasan por el gateway (presupuesto, breaker, registro de uso) pero sin reintentos: una credencial mal cargada debe fallar rápido y con su mensaje. */
  private ProviderLlmGateway.Reply viaGateway(Provider provider, String model, String message, int timeoutSeconds, java.util.function.Supplier<ProviderLlmGateway.Reply> call) {
    return executor.run(new GatewayExecutor.Spec<>(ModelFunction.EVALUATOR, provider.name(), model, GatewayUsageLog.estimateTokens(message), java.time.Duration.ofSeconds(timeoutSeconds), call,
        reply -> new int[] {reply.inputTokens(), reply.outputTokens()}, reply -> { }, "El proveedor no respondió a tiempo", "El proveedor no pudo responder la prueba", IllegalStateException::new, false));
  }
  ...
  @PostMapping("/provider-credentials/{id}/test-model") public ChatResponse testModel(@PathVariable UUID id,@Valid @RequestBody CreateDeployment request,@RequestHeader HttpHeaders headers) { authorization.require(headers); var credential=credential(id); var reply=viaGateway(credential.provider(),request.modelId().trim(),"Respondé únicamente: conexión operativa.",60,() -> gateway.chat(credential.provider(),credential.baseUrl(),crypto.decrypt(credential.encryptedSecret(),credential.nonce()),request.modelId().trim(),"Respondé únicamente: conexión operativa.")); return new ChatResponse(reply.text(),reply.inputTokens(),reply.outputTokens()); }
