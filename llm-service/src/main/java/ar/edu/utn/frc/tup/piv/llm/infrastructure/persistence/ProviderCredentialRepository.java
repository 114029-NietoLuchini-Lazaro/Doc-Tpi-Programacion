...
public class ProviderCredentialRepository {
  ...
  /** Resolves a configured function to a real enabled deployment, without a global active-model fallback. */
  public Optional<Deployment> forProviderModel(Provider provider, String modelId) { return jdbc.query("select d.id,d.credential_id,c.provider,c.display_name,d.model_id,d.evaluator_state::text,d.created_at,d.candidate_slot,d.chat_verified_at from llm.model_deployments d join llm.provider_credentials c on c.id=d.credential_id where c.provider=? and d.model_id=? and d.evaluator_state <> 'DISABLED' and d.candidate_archived_at is null order by case when d.evaluator_state='ACTIVE' then 0 else 1 end, d.created_at desc limit 1", (rs,row) -> deploymentRow(rs), provider.name(), modelId).stream().findFirst(); }
  public Optional<Deployment> calibrationTarget() { return jdbc.query("select d.id,d.credential_id,c.provider,c.display_name,d.model_id,d.evaluator_state::text,d.created_at,d.candidate_slot,d.chat_verified_at from llm.model_deployments d join llm.provider_credentials c on c.id=d.credential_id where d.calibration_target=true and d.candidate_archived_at is null", (rs,row) -> deploymentRow(rs)).stream().findFirst(); }
