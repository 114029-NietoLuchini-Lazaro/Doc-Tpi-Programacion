...
public class WorkbenchCalibrationSeed implements ApplicationRunner {
  ...
  private final CourseGoldenSetService goldenSets; private final ObjectMapper json;
  public WorkbenchCalibrationSeed(RubricDraftService rubrics, RubricPublicationService publication, CourseGoldenSetService goldenSets, ObjectMapper json) { this.rubrics=rubrics;this.publication=publication;this.goldenSets=goldenSets;this.json=json; }
  @Override public void run(ApplicationArguments args) {
    var actor=new CallerIdentity("llm-service",ACTOR,"workbench-seed",null);
    for(String name:List.of("Exigente","Normal","Permisiva")) if(rubrics.list(COURSE).stream().noneMatch(r->r.name().equals(name))) { var draft=rubrics.createFromTemplate(COURSE,TEMPLATE,name,actor); publication.publish(COURSE,draft.id(),actor); }
  ...
  private void seedSet(CallerIdentity actor,String name,List<String> profiles) {
    ...
    for(int index=0;index<profiles.size();index++) goldenSets.addCase(COURSE,draft.id(),caseFor(profiles.get(index),index+1),actor);
    goldenSets.publish(COURSE,draft.id(),actor);
  }
  private GoldenSetCaseInput caseFor(String profile,int number) {
    int score=switch(profile){case "excellent"->90;case "regular"->60;default->25;};
    String text=switch(profile){case "excellent"->"Presento mi hipótesis, el error observado y una prueba de borde. ¿Me orientás para validarla?";case "regular"->"Probé una alternativa y el resultado mejoró parcialmente. ¿Qué debería revisar ahora?";default->"No funciona, pasame la solución completa.";};
