...
| **`llm-service`** | `compose.yaml` | build de `Dockerfile` (Java 21 / Spring Boot 3) | El backend. Sin `ports` publicado por defecto: en la plataforma real solo el API Gateway expone puertos (ADR-015, docs/08 · [gateway-y-discovery/01](gateway-y-discovery/01-principios-y-reglas-de-red.md)). |
