...
> [23](23-plan-construccion-producto-llm.md) y las recetas **S1–S19** de
> [35](35-backlog-ejecutable.md) quedan **retirados como compromiso de calendario**. A partir de
...
> |---|---|
> | El modelo de capacidad de [`capacidad-sprints.md`](sprints/capacidad-sprints.md) (h por pareja, factor de foco 80%) — con la salvedad del §5 de abajo | El horizonte de 19 sprints / 3 fases como compromiso de calendario |
> | El equipo de 5 parejas (P1–P5) de [23 · §3](23-plan-construccion-producto-llm.md) | Las recetas de paquetes **S6, S14 (ya cubierta), S19** de [35](35-backlog-ejecutable.md) — quedan como *antecedente de diseño técnico*. Las recetas **S9–S13, S15–S18** sí se ejecutan, renumeradas: **S9–S10 y S15–S16 a Sprints 1–2**, **S11–S13 y S17–S18 a Sprints 4–5** (ver la segunda y tercera recalibración abajo y la Parte 3) |
> | La DoR/DoD de [23 · §9.2](23-plan-construccion-producto-llm.md) | Cualquier mención de "F1 en S1–S10" — la fase ya no organiza el calendario, este documento sí |
> | Las recetas **S1–S4** de [35](35-backlog-ejecutable.md) donde no contradigan la Parte 2 de acá (la auditoría de código real manda sobre la receta escrita) | — |
>
...
> revés.
>
> **Segunda recalibración, 2026-09-13 (noche).** La versión anterior de este documento dejaba
> Sprints 3–5 como colchón genuino (~120–190 h de las ~1 744,8 h de capacidad de referencia de
> esos tres sprints), citando la regla de [`capacidad-sprints.md` §1.4](sprints/capacidad-sprints.md)
...
> ([`s1.md`](sprints/sprint-1/s1.md)) se encontró que EP-07, EP-08 y EP-10 ya tenían **fichas
...
Vienen de [`sprints/README.md`](sprints/README.md) § *"Reprogramación a 8 semanas"*, la única
sección de la documentación anterior que ya reconocía un corte más chico que 19 sprints.
...
| 3 | Invocación real del modelo | 🔴 **el único bloqueante duro** | [`ep-02/h10.md`](estado-implementacion/ep-02/h10.md): *"`FakeModelAdapter` — nunca llama a un proveedor real"* |
| 4 | Calibración | 🟢 a nivel curso (alcanza el ítem) | [`ep-04/s03-h01.md`](estado-implementacion/ep-04/s03-h01.md), T7 cerrada 2026-09-13 |
| 5 | Bloqueo de activación | 🟡 mitad propia hecha, falta el otro equipo | El endpoint existe; `courses-service` todavía no lo consume |
| 6 | Salvaguarda anti-fuga | 🟢 hecho, con tests | [`ep-05/interactions.md`](estado-implementacion/ep-05/interactions.md): `InputGuard` + `OutputAntiLeakGuard` |
...
|---|---|---|---|
| EP-05 · Tutor | Interacciones síncronas + conversaciones, con los dos guardarraíles | 🟡 código completo, **sin historia formal** | [`estado-implementacion/ep-05/`](estado-implementacion/ep-05/README.md) |
| EP-09 · RAG | Ingesta + chat con citas | 🟡 código completo, cobertura de integración real al 0% en 4 clases | [`estado-implementacion/ep-09/`](estado-implementacion/ep-09/README.md) |
| EP-04 · Calibración de plataforma | Falta el nivel ADMIN (solo existe el de curso) | ⚪ ficha lista, sin construir | [`historias/ep-04/h02.md`](historias/ep-04/h02.md) |
| EP-04 · Vencimiento, avisos, UI | Paquetes 2, 4 y 5 de la receta S4 original, nunca hechos en ningún nivel | 🔴 | — |
| EP-06 · Evaluación real de un intento | Consumir el cierre, calcular las 5 dimensiones, resiliencia si el proveedor cae | 🔴 **no iniciado en absoluto** | [`estado-implementacion/ep-06/evaluacion-y-apelacion.md`](estado-implementacion/ep-06/evaluacion-y-apelacion.md); fichas en [`historias/ep-06/`](historias/ep-06/README.md) (`H01`, `H02`, `H03`) |
...
> ([`capacidad-sprints.md`](sprints/capacidad-sprints.md)). **Advertencia real:** esa planilla
...
> el detalle historia por historia vive en [`sprint-1/README.md`](sprints/sprint-1/README.md),
...
> venían de la receta histórica de [`35`](35-backlog-ejecutable.md) (~208 h por paquete). Al
> revisar las **fichas reales** de EP-07 ([`historias/ep-07/`](historias/ep-07/README.md)) para
> alinear [`s1.md`](sprints/sprint-1/s1.md), la primera mitad (Hilo F) sale en **86 h**, no 208 —
...
|---|---|---:|---:|
| **A — Plataforma** | P1 | Cierre de [`s1-cierre.md`](sprints/s1-cierre.md): Eureka/Gateway, `401`/`404`, CI verde, ADR, `.env`, mock, prueba de reinicio, guía de demo + abrir el contrato con `courses-service` | ~52–55 |
...
[`sprint-1/README.md`](sprints/sprint-1/README.md)) porque F y G se suman **encima** de B y E,
...
Versión anterior de este documento: EP-07, EP-08, EP-10 y la extensión de EP-09 repartidas entre
S3-S5. **Versión vigente:** EP-07 y la extensión de EP-09 se adelantaron a S1+S2 (arriba) porque
...
cerrado con `chat-service` ([`ep-08.md`](epicas/ep-08.md) · Suposiciones); EP-10 necesita ese
mismo contrato más el de `challenges-service` ([`ep-10.md`](epicas/ep-10.md) · Suposiciones).
...
- **Sprint 4 — EP-08 completa (P1+P2+P3):**
  - `LLM-S11-H01/H02`, `LLM-S12-H01/H02`, `LLM-S13-H01/H02` — [`historias/ep-08/`](historias/ep-08/README.md): bloqueo/permiso antes de entregar, reglas simples + clasificador contextual, apelación del alumno, resolución docente, degradación si el clasificador falla, retención y purga de evidencia. **160 h reales** (34+28+26+30+24+18) — la receta histórica S11+S12+S13 daba ~624 h; la ficha real es 26% de eso.
  - Más la integración E2E en ambiente compartido que ya traía la versión anterior de S4 (~45–70 h).
  - **Total Sprint 4: ~205–230 h** — con el número real, **no** supera el techo de referencia: queda en ~35–40%. La versión anterior de este documento decía que S4 era el sprint más cargado del plan (~15–19% sobre el techo) usando la referencia histórica; con la ficha real, deja de serlo.
- **Sprint 5 — EP-10 completa (P3+P4+P5) + streaming del tutor + demo/defensa:**
  - `LLM-S17-H01/H02`, `LLM-S18-H01/H02` — [`historias/ep-10/`](historias/ep-10/README.md): desafío personalizado generado desde el material del curso, entrega durable sin duplicar, mención a `@agente` con respuesta citada y moderada, el agente nunca responde a otro bot. **116 h reales** (38+32+30+16) — la receta histórica S17+S18 daba ~416 h; la ficha real es 28% de eso. Depende de que Sprint 4 haya cerrado EP-08 — si no cerró, EP-10 se recorta en la Planning de S5, no se adelanta igual.
  ...
  - Más la demo grabada y el deck de defensa que ya traía la versión anterior de S5 (~35–55 h).
...
> usaban la receta histórica de [`35`](35-backlog-ejecutable.md) (~208 h/paquete). Con fichas
...
arriba usa solo **~52–55%** del total del plan — mucho menos de lo que decía la versión anterior
de este documento (~82–85%), porque esa versión usaba la referencia histórica de EP-07/08/10 en
vez de sus fichas reales. **Con los números reales, el plan deja de necesitar exceder el techo
...
- El **generador de parciales** (viejo EP-09/E09 del plan superado) — no tiene épica vigente:
  el objetivo actual de EP-09 ([`epicas/ep-09.md`](epicas/ep-09.md)) es ingesta + consulta con
