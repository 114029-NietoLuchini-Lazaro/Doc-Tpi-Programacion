...
> [38 — Plan vigente: máximo 5 sprints](05-plan-de-cinco-sprints.md). Las recetas **S1–S4** de acá
...
Cada bloque de S1 en adelante es una receta de implementación. Las horas de cada paquete son el **trabajo estimado** (desarrollo, pruebas, revisión, documentación y demo; no reuniones ni reserva) y suman **~208 h por sprint**. Esa cifra es una estimación gruesa anterior a los contratos, **no el tope**: la **capacidad del sprint es 571 h** ([23 · §2.1](03-plan-de-construccion-del-producto.md)) y la diferencia (**~363 h**) es margen para re-estimar en Planning, imprevistos y coordinación de 12 personas. Reestimar en cada Planning según disponibilidad real. Leer antes el [playbook](06-playbook-de-construccion.md)

> **Excepción S1/S3.** En la reprogramación a 8 semanas ([sprints/README.md](09-epicas-historias-tareas-sprints/sprints/README.md)
...
Este documento tiene tres capas: las **épicas** (para qué producto), el **Sprint 0** (qué se deja listo antes de construir) y las **recetas S1–S19** (cómo se construye cada incremento). La DoR y la DoD únicas están en [23 · §9.2](03-plan-de-construccion-del-producto.md); el cálculo de capacidad, en [23 · §2](03-plan-de-construccion-del-producto.md). La **vista de entrega** que reúne Sprint 0, DoD, capacidad, épicas e historias de S1 con el formato de la guía de cátedra es [30](02-arranque-agil-y-sprint-0.md)
...
> Los números de RF son orientativos. La traza fina historia → requisito se mantiene en [21 · matriz de trazabilidad](../00-gobierno-y-evolucion/04-matriz-trazabilidad.md). Las parejas P1–P5 se definen en [23 · §3](03-plan-de-construccion-del-producto.md)

> **Cómo se pasa cada épica al template de Taiga.** La columna *Resultado que habilita* de esta tabla es el **Objetivo** del [template de Épica](10-plantillas/epica-taiga.md); *Sprints* y *Pareja líder* son contexto de planificación; los apartados *Suposiciones y Restricciones*, *Criterios de Aceptación a nivel Épico* y *Dependencias / Impactos* se completan en la ficha (las dependencias arrancan de [23 · §8](03-plan-de-construccion-del-producto.md)). El template de épica **no** usa Como / Quiero / Para ni BDD —a diferencia de lo que sugiere [29 · §4](08-guia-de-historias-de-usuario.md)para épicas en general—: para la entrega manda el template oficial. Las diez fichas ya redactadas, una por archivo, están en [`docs/epicas/`](09-epicas-historias-tareas-sprints/epicas/README.md)

> **Publicación en Taiga.** Este documento es la versión de trabajo. Las épicas y las HU se cargan además en el backlog de Taiga con los *Templates de Épica y de Historia de Usuario* de la Wiki, y se referencian desde las páginas de la Wiki por su **enlace permanente** (ver [27 · Guía de la Wiki](07-guia-wiki-taiga.md)). El método para redactarlas y estimarlas es [29 · Guía de cátedra: Historias de Usuario](08-guia-de-historias-de-usuario.md)

---

## Sprint 0 — Arranque del proyecto

Sprint 0 **no produce incremento de software** ni consume la capacidad de entregables: es la primera Planning ampliada, con acuerdos y preparación de ambiente. Ocurre **antes de S1** y deja cumplida la DoR de todas las historias de S1. No se compromete ninguna historia ni se asignan puntos, salvo estimar la historia canónica. El **acta para completar** el día del Sprint 0 está en [`docs/sprints/sprint-0.md`](09-epicas-historias-tareas-sprints/sprints/sprint-0/sprint-0.md)
...
- [ ] **Cálculo de capacidad de S1** hecho con la fórmula de abajo a partir de la planilla de disponibilidad y registrado en una copia de la [plantilla de sprint](10-plantillas/sprint-llm.md)
- [ ] **DoR y DoD** ([23 · §9.2](03-plan-de-construccion-del-producto.md)) leídas y aceptadas por el equipo.
- [ ] **Historia canónica** del equipo elegida y estimada ([29 · §5.3](08-guia-de-historias-de-usuario.md)): una HU pequeña, entendida por todos, que fija la referencia de puntos para todo el backlog. Candidata natural: `LLM-S01-H06` (consultar golden set tras reinicio).
- [ ] Épicas revisadas; cada historia de S1 asignada a una épica (ver tabla de S1).
- [ ] Épicas e historias de S1 cargadas en el backlog de Taiga con los templates oficiales ([HU](10-plantillas/historia-de-usuario-taiga.md)con BDD de camino feliz + 2 negativos, puntos Fibonacci contra la canónica e INVEST verificado; [épica](10-plantillas/epica-taiga.md) con objetivo y CA a nivel épico); permalinks anotados para referenciarlas en la Wiki.
...
- [ ] Remoto `tpi-llm` con `main` y `develop` creadas y protegidas (PR obligatorio, CI, sin push directo ni force-push) según [GITFLOW](../09-flujo-de-trabajo-del-equipo/01-gitflow.md)y el [flujo diario y Git](../09-flujo-de-trabajo-del-equipo/02-workflow-diario.md). Las `feature/sNN/llm-sNN-hNN-<slug>` nacen de `develop` al arrancar cada historia de S1, no en Sprint 0.
...
S1 es un sprint de arranque: **casi todo es habilitador técnico**. Sólo H05–H07 son **HU de valor** en el sentido de [29 · §3](08-guia-de-historias-de-usuario.md) (un docente percibe el resultado). H01–H04, H08 y H09 fallan la **V** de INVEST: son **tareas de sprint** con enunciado en primera persona del equipo, no HU de valor. Para el backlog de Taiga:

- **H05, H06, H07** se cargan como **HU** con el [template oficial](10-plantillas/historia-de-usuario-taiga.md): Como/Quiero/Para con rol real (docente), BDD de camino feliz + 2 negativos, puntos Fibonacci.
...
| **LLM-S01-H09** | Como equipo, quiero la suite de pruebas y la guía de demo de S1 para que la Review se valide con evidencia y no con relato | Unitarias + Testcontainers/Flyway + WireMock del Gateway; reinicio de Compose probado; cobertura según [24](../06-operacion-calidad-y-pruebas/02-convenciones-de-cobertura.md). Guía de demo reproducible: acceso autorizado → carga → consulta tras reinicio | EP-01 | todos | H04–H07 | 18 |
...
**Demo de S1:** un docente autorizado crea un golden set, carga una entrada y la consulta después de reiniciar el servicio. **H10 no es parte de esta demo** — corre en paralelo como preparación de EP-02 (reprogramación a 8 semanas, ver [`sprints/README.md`](09-epicas-historias-tareas-sprints/sprints/README.md)
...
> **propagar o revertir** que cierra [I-10](../contracts/90-mapa-de-integracion-historico.md) y decide si la adenda
> [`docs/contracts/llm-service-v1-tutor-sse-adenda.md`](../contracts/historicos-y-contratos-v1/llm-service-v1-tutor-sse-adenda.md)
...
> **Antecedente de diseño técnico** ([38 · nota introductoria](05-plan-de-cinco-sprints.md)): esta
> receta se ejecuta renumerada como la primera mitad de EP-07 en Sprint 1. La granularidad por
> alumno (cantidad de usos y tokens/día, propuesta en [08 P-12](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
