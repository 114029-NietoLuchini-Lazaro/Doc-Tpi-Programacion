...
> [38 — Plan de 5 sprints](05-plan-de-cinco-sprints.md) (retira el horizonte de 19 sprints que
> planteaba [23](03-plan-de-construccion-del-producto.md)). Para capacidad, responsables y
> ceremonias siguen valiendo [23](03-plan-de-construccion-del-producto.md) y su
> [plantilla de sprint](10-plantillas/sprint-llm.md): 12 integrantes, cada sprint presupone
...
> **Rebase obligatorio antes de planificar.** Este backlog es una estimación anterior a los contratos v1. La selección de historias debe respetar [21](../00-gobierno-y-evolucion/04-matriz-trazabilidad.md): excluir Fase 2/Fase 3, usar Kafka para eventos y reemplazar el dispatcher `/ai/{funcion}` por recursos bajo `/api/llm/**`.
...
> [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md), los siete bloques de entregables y los catorce pasos de
> [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md), los seis endpoints y los eventos de
> [18](../contracts/91-contratos-inter-equipos-historicos.md), y los bloqueantes de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
>
> **Este documento no decide nada nuevo.** Si algo acá contradice a un ADR de
> [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md) Parte A, manda el ADR.
...
El reparto de [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md) Parte 2 es de seis personas, una por módulo. Con doce,
...
Son las de [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md) Parte 2 §7, con la corrección que impone ser doce:
...
3. **`repository/` y `entity/` son territorio compartido y el punto de conflicto más probable**
   ([ESTRUCTURA.md](../06-operacion-calidad-y-pruebas/04-estado-de-implementacion/codigo-ejemplo/fuentes/ESTRUCTURA.md), preservado tal
...
| E01-05 | Errores tipados según el contrato | Los cinco códigos de [18](../contracts/91-contratos-inter-equipos-historicos.md) §1.5 —`429`, `503`, `409`, `422`, `401`— salen con su `codigo` y su `trace_id`. Un test por código | 3 | E01-04 | 🔴 |
...
| E01-08 | Publicación de eventos al bus | Los cuatro eventos de [18](../contracts/91-contratos-inter-equipos-historicos.md) §2 salen con el schema acordado. Sin bus disponible se publican contra un stub local y quedan en la tabla de salida | 5 | E01-03 | 🔴 |
...
| E02-07 | Cuotas por usuario, por desafío y por día — más, si el back office (Tema 12) lo necesita, un techo por alumno (cantidad de usos y tokens/día, ver [08 P-12](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)) | Al agotarse, `429` con `cuota_agotada` (RF-IA-22). Los umbrales van en tabla porque P-05 sigue abierta | 5 | E02-06 | 🔴 |
...
| E02-11 | Rate limiting en tres capas | Bucket4j en el borde, cuota por desafío y Resilience4j sobre el proveedor, según [19](../04-seguridad-datos-y-cumplimiento/03-rate-limit-y-resiliencia.md). Un test dispara *Denial of Wallet* y el costo queda acotado | 5 | E02-07 | 🔴 |
...
| ID | Historia | Criterio de aceptación | SP | Dep. | Prio |
|---|---|---|---|---|---|
| E03-01 | OpenAPI de los seis endpoints, publicado antes de implementar | El archivo está en el repo, cubre los seis de [18](../contracts/91-contratos-inter-equipos-historicos.md) §1.1 y **ningún endpoint más**. Cualquier otro es diseño interno | 5 | — | 🔴 |
...
| E03-09 | Autenticación servicio a servicio | Token técnico y validación RS256 según [gateway-y-discovery/04](../02-arquitectura-y-plataforma/06-gateway-y-discovery/04-seguridad-y-pipeline-de-filtros.md)5 | E03-07 | 🟡 |
...
| ID | Historia | Criterio de aceptación | SP | Dep. | Prio |
|---|---|---|---|---|---|
| E04-01 | Ingesta de PDF a texto, incluidos los que traen imágenes | Un apunte real de la cátedra queda en texto, con el camino de OCR resuelto para las páginas escaneadas, según [12](../03-capacidades-de-ia/rag-e-ingesta/01-almacenamiento-e-ingesta.md) | 8 | — | 🔴 |
...
| E05-08 | Publicar `score_de_ia_calculado` y `score_pendiente_diferido` | Con el schema de [18](../contracts/91-contratos-inter-equipos-historicos.md) §2.1 y §2.2. El segundo se emite cuando el proveedor está caído | 3 | E01-08 · E05-05 | 🔴 |
| E05-09 | Pruebas de algo no determinístico | Snapshots con tolerancia por dimensión, según [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md). Cambiar un prompt sin correr esto queda prohibido — es regla del equipo | 5 | E05-03 | 🟡 |
...
| E06-08 | Eventos `calibracion_aprobada` y `calibracion_fuera_de_tolerancia` | Con el schema de [18](../contracts/91-contratos-inter-equipos-historicos.md) §2.3, consumidos por el Tema 02 y el Tema 12 | 3 | E01-08 · E06-05 | 🟡 |
...
| E07-04 | Registro de incidentes y evento de jailbreak | Cada incidente queda registrado y publica `incidente_de_jailbreak` (RF-IA-10) con el schema de [18](../contracts/91-contratos-inter-equipos-historicos.md) §2.4 | 5 | E01-08 | 🟡 |
...
| E08-06 | 🔴 Rotar la API key de Groq que estuvo versionada en la demo | El archivo (`lara-heredia-demo-llm-spring-ai`) ya no está en el working tree, pero la key puede seguir en el historial de git — rotarla en el proveedor. Detalle en [CORRECCIONES-SUGERIDAS.md](../06-operacion-calidad-y-pruebas/04-estado-de-implementacion/codigo-ejemplo/fuentes/CORRECCIONES-SUGERIDAS.md) | 2 | — | 🔴 |
...
| E12-08 | Sesión de integración con la agenda de ocho ítems | Los ocho ítems de [18](../contracts/91-contratos-inter-equipos-historicos.md) §6 tratados, con acuerdo escrito por cada uno | 3 | 🔴 |
| E12-09 | Los 31 ítems de contenido de ejemplo pasan a definitivos | El inventario de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md) Parte C queda sin marcas de *ejemplo*: cada célula define los suyos | 8 | 🟡 |
...
| — | Agente `@mención` (RF-CHT-05) | Fase 3. Está diseñado en [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 4 y ahí se queda hasta que alguien lo priorice | Fase 3 |
...
> [23 · §9.2](03-plan-de-construccion-del-producto.md) (DoR de 9 puntos con BDD, DoD por historia y
> por incremento); el [playbook `36`](06-playbook-de-construccion.md)
> [30 · §2](02-arranque-agil-y-sprint-0.md) remiten a esa misma fuente. Lo de abajo se conserva
...
4. Se sabe qué documento de `docs/` la respalda. Una historia sin respaldo es una decisión nueva
   disfrazada de tarea, y las decisiones se toman en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md), no en el
   tablero.
...
5. Si cambia una decisión, quedó anotada en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
> de 6 células. El plan vigente es [23 · §2.2](03-plan-de-construccion-del-producto.md): 12
...
| **Bloqueos externos abiertos** | Los 🔴 de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md) que siguen sin dueño | Cualquiera que sobreviva a dos sprints |
