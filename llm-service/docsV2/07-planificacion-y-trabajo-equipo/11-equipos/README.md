...
>
> - **`contratos.md`** — puntero al contrato canónico en `docsV2/contracts/equipos/`; no se edita como
...
>   con ellos.
>
> **Qué es esto ahora.** Desde la consolidación del 2026-09-15, cada contrato completo vive en
...
> |---|---|
> | [18 — Contratos inter-equipos](../../contracts/91-contratos-inter-equipos-historicos.md) | §0 (cómo leemos los contratos, reglas de todos), el anexo histórico y un índice corto hacia cada carpeta — el detalle por equipo (§4.1-4.8 viejas) ya se movió acá |
> | [17 — Mapa de integración](../../contracts/90-mapa-de-integracion-historico.md) | Los 3 diagramas de secuencia siguen también acá (vista cruzada para comparar los tres presupuestos de latencia a la vez) — copiados además en `tema-05` y `tema-11`. La tabla I-01 a I-16: las filas "decide: Nosotros" quedan internas, las de Product Owner y las transversales ya están repartidas |
...
> | [`contracts/historicos-y-contratos-v1/`](../../contracts/historicos-y-contratos-v1) | Los YAML V1 históricos (OpenAPI/AsyncAPI); los schemas vigentes se consultan en [`contracts/`](../../contracts/README.md) y nunca se copian en los contratos narrativos |
>
> **Cómo se actualiza.** Cuando algo se acuerde (por ejemplo en la sesión de integración), se
> mueve de `pendientes.md` a la vista canónica de `contracts/equipos/` de ese equipo — ya no hace
> falta además actualizar `17`/`18`, salvo que el ítem sea transversal a varios equipos a la vez
...
- **Formato de error:** RFC 7807 Problem Details (respuesta `Problem` del
  [`llm-service-v1.openapi.yaml`](../../contracts/historicos-y-contratos-v1/llm-service-v1.openapi.yaml)), con
