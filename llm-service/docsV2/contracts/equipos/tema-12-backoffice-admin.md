...
> **Fuente ejecutable:** [`../llm-service.openapi.yaml`](../llm-service.openapi.yaml). Las rutas
> `/ai/**` y los ejemplos V1 que siguen son históricos; para integrar Backoffice se debe usar el
> OpenAPI V2 canónico.
...
- `GET /ai/calibracion/{curso_cohorte_id}` → `GET /api/llm/course-cohorts/{courseCohortId}/calibration`
  ...
  [`tema-02-cursos-y-matricula.md`](tema-02-cursos-y-matricula.md)).
...
- 🟡 **Propuesta** — `PUT /api/v1/operations/quotas/student/{studentId}` — fijar el límite de uso
  ...
  `LLM-S09-H02` ([`historias/ep-07/h02.md`](../../07-planificacion-y-trabajo-equipo/09-epicas-historias-tareas-sprints/historias/ep-07/h02.md)); pendiente de
  confirmar con Product Owner y DPO ([08 P-12](../../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)).
...
[`tema-03-motor-de-desafios.md`](tema-03-motor-de-desafios.md) — mismo
...
que el evaluador ([`tema-03-motor-de-desafios.md`](tema-03-motor-de-desafios.md)):
...
- 🟡 Propuesto — Tema 12 es dueño de la pantalla de límites de uso de IA por alumno (cantidad de
  usos y cantidad de tokens por día). Ver [`pendientes.md`](../../07-planificacion-y-trabajo-equipo/11-equipos/tema-12-backoffice-admin/pendientes.md) por la tensión con la
