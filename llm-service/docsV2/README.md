# docsV2 — guía de entrada a `llm-service`

Esta es la documentación de trabajo de Tema 07. Si te incorporás al equipo, empezá acá: no hace
falta recorrer `docs/` (V1) ni buscar documentos por nombre. `docs/` se conserva como antecedente
de consulta; las reglas, contratos y correcciones que se aplican al trabajo actual viven en V2.

> La implementación que esta documentación describe se verifica contra el `main` local. El
> Workbench es sólo consumidor de referencia: en el laboratorio usa `gateway-mock` y
> `courses-mock`, mientras `llm-service` y PostgreSQL se ejecutan de verdad. Ver
> [laboratorio de integración local](02-arquitectura-y-plataforma/07-laboratorio-integracion-local.md).
...
- **Histórico o evidencia:** los nombres `historico`, `historicos-y-contratos-v1`, `registro`,
  `estado-de-implementacion` e `investigacion-y-material` conservan contexto. No se editan para
  cambiar una regla vigente.

No modifiques `docs/` para corregir una decisión actual. Actualizá el documento canónico V2,
