...
> (ítems E-07 y E-08 en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md), Parte C).
...
- `curso_id` es lo que hace cumplir **RF-IA-06** (perímetro temático) y aísla cursos entre sí. Se
  filtra en el servidor, nunca desde el cliente. Ver [05](../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md)
...
Ver [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) §7.
...
4. **Es lo que hace aceptable un modelo barato.** El escenario B de [03](01-modelos-costos-y-contexto.md) usa
...
la corrección del contenido. La decisión P-01 de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md) cerró ese punto.
...
| **Respuesta corta (clave única)** | ❌ **No** | Normalizar (mayúsculas/tildes/espacios) + distancia de edición contra la(s) respuesta(s) aceptada(s) — ver [`docs/entregas/recomendacion-correccion-respuestas-cortas.md`](../01-vision-alcance-y-entrega/03-entregas/recomendacion-correccion-respuestas-cortas.md)
...
[P-01](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md#-p-01--corrector-de-respuestas-abiertas-fuera-de-alcance) para
...
Desarrollo completo en [05](../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md)

## 7. Decisiones que hay que tomar

Registradas en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
| 3 | ¿Qué pasa si el moderador se cae? El PRD no lo dice | Ver [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) §5 |
...
subestimada de las siete. Ver [01](../01-vision-alcance-y-entrega/01-problema-y-alcance.md) §3.2.
...
| Costo por llamada | **USD 0** — no hay tokens que facturar (ADR-012), ver [03](01-modelos-costos-y-contexto.md) | Del orden del tutor |
...
3. **El moderador no tiene ningún LLM adentro.** Ninguna de las dos piezas escribe texto. La factura
   del moderador en [03](01-modelos-costos-y-contexto.md) es **USD 0**, y no es una estimación
...
2. **Datos** — cuantos menos mensajes de alumnos salgan del sistema, mejor ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md)
...
| `RateLimiter` de **Resilience4j** | Librería ya elegida en [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) | 🟩 Maven | Spam por frecuencia (token bucket) |
...
⚠️ **No hay que escribirlo.** [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) ya eligió **Resilience4j** para
...
2. **Fijar los dos cortes.** El diccionario da un score continuo de 0 a 1; RF-CHT-11 pide tres
   ...
   de producto que sigue abierta en la Parte C de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
| **Qué pasa si el proveedor se cae** | **Circuit Breaker** | 🟢 **Ya elegido**: Resilience4j, en [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md). Es lo que implementa el fail-open de §2.4 |
...
> Idempotent Receiver vienen de ADR-001 y de [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)—, así que lo que este
...
| **Los datos salen** | Mensajes de alumnos viajan a un tercero ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md)) | Cuanto más resuelve la capa clásica, menos sale. Y hay salida local: ver abajo |
...
retira y se genera el incidente. El fundamento está en [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md)

Es **decisión del Product Owner**: P-02 en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
está retenido y por qué. Ver [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md)
...
[`contracts/llm-service-v1-moderacion-borrador.yaml`](../contracts/historicos-y-contratos-v1/llm-service-v1-moderacion-borrador.yaml)
(borrador sin implementar, portado de `codigo-ejemplo/ms-evaluacion-llm` antes de eliminar esa
carpeta — ver [`docs/estado-implementacion/README.md`](../06-operacion-calidad-y-pruebas/04-estado-de-implementacion/README.md)
Es un recorte del sobre común `POST /ai/{funcion}` de [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) —a propósito,
...
[03](01-modelos-costos-y-contexto.md). Es el golden set del moderador, en chico — y es **mucho más
...
borre. Va a la Parte C de [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
Las tres quedan anotadas como P-09, P-10 y P-11 en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
