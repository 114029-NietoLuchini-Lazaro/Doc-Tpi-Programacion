...
> **Identidad actual.** En los diagramas heredados, `ms-evaluacion-llm` se reemplaza conceptualmente por `llm-service`. Solo el API Gateway expone `/api/llm/**`; Nginx no rutea directamente al microservicio.
...
negociables gobiernan [02](01-arquitectura-y-stack.md) §1. Y llega desde el otro lado del sistema:
...
[04](../03-capacidades-de-ia/02-funciones-de-ia.md) ya validaba contra el golden set; el shadow le agrega el tráfico real.

## Y dos cosas que ya hacíamos sin nombrarlas

**Feature flags.** *"Cambiar de modelo = editar una fila. Sin deploy"*
([02](01-arquitectura-y-stack.md)) y *"si lo cambia un ADMIN sin pedir un deploy, va en la base"*
([06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md)). Eso **es** un feature flag, con persistencia en tabla.
...
| **Reverse proxy** | nginx delante del API Gateway | ✅ Es la arquitectura real, ahora explícita en [02](01-arquitectura-y-stack.md) §2 |
...
| **`expose` vs `ports`** | *"Sin puerto publicado"* ya estaba en [01](../01-vision-alcance-y-entrega/01-problema-y-alcance.md) | 🟡 Ya lo hacíamos; la unidad le da el mecanismo exacto |
...
| **Secretos (Vault)** | Los tres niveles están en [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 5 | 🟡 Coincidimos; la herramienta concreta sigue sin dueño |
| **Prometheus / Grafana** | Micrometer + Actuator, elegidos en [02](01-arquitectura-y-stack.md) | 🟡 Compatibles: Micrometer exporta a Prometheus |

## 4. ✅ Lo que adoptamos

| # | Qué | Por qué importa | Dónde va |
|---|---|---|---|
| **A-1** | 🔴 **El proveedor de LLM fuera del readiness** | Una sonda que lo incluyera sacaría la instancia de rotación cuando el proveedor cae — **justo lo que RF-IA-27 prohíbe**. Convierte una degradación prevista en una caída total | ADR-014 · [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-2** | 🔴 **Shadow deployment para calibrar** | Validar una `rubric_version` o un `prompt_version` nuevos contra transcripciones reales **descartando la salida**. El riesgo que la unidad marca —duplicar operaciones que escriben— no nos aplica: la evaluación en sombra no emite score | [04](../03-capacidades-de-ia/02-funciones-de-ia.md))
| **A-3** | **Rolling update como estrategia declarada** | No cambia lo que hacemos: hace explícito el costo. Convierte el versionado del contrato de recomendación en requisito de despliegue | ADR-013 · [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-4** | **`healthcheck` + `condition: service_healthy`** | `depends_on` solo espera a que el contenedor inicie. Sin esto el arranque en frío falla intermitentemente y siempre parece otra cosa | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-5** | **Build multietapa en el Dockerfile** | Ni Maven ni el código fuente llegan a la imagen que corre. El Dockerfile todavía no existe: se escribe con esta regla desde el primer día | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-6** | **Staging como tercer entorno** | La Parte 5 hablaba solo de dev y producción. Staging es el que le da sentido a *"la misma imagen viaja a los tres"* | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-7** | **Tag por SHA del commit, nunca `latest`** | Un `latest` no se puede revertir porque no nombra nada | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| **A-8** | **El vocabulario** | Reverse proxy, BFF, rolling update, feature flag, shadow, readiness. Aparece en la defensa | [11](../00-gobierno-y-evolucion/03-glosario-y-metadata.md)
...
| **Terraform / Ansible** | No hay infraestructura que provisionar: los entornos son los tres de [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 §5 y se administran a mano |

## 6. Lo que aportamos que la unidad no cubre

| # | Qué | Por qué importa |
|---|---|---|
| **B-1** | 🔴 **El techo del escalado no son las réplicas: es la cuota del proveedor** | La unidad razona en un mundo donde sumar instancias siempre suma capacidad. En [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 2 el cuello de botella está afuera del sistema: se pueden levantar seis workers y seguir esperando lo mismo |
...
| **B-4** | **El rollback de código no repara una nota emitida** | Las notas son append-only ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) §3.3) y RF-IA-13 prohíbe recalcular puntajes históricos. Revertir el binario detiene el daño y no lo deshace: eso es un override de docente, que es otro camino |
...
| # | Acción | Dónde |
|---|---|---|
| 1 | Los tres «gateway» separados, y la octava colisión declarada | [02](01-arquitectura-y-stack.md))
| 2 | El borde nginx en el diagrama de sistema | [02](01-arquitectura-y-stack.md) §2 |
| 3 | Estrategia de despliegue, rollback, ventana post-release, sonda de salud y staging | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 |
| 4 | ADR-013, ADR-014 y ADR-015 | [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
| 5 | El hueco de CD declarado | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 7 §6 |
| 6 | Corregido: la tabla *"Qué se despliega"* decía **Python FastAPI** contra ADR-005 | [02](01-arquitectura-y-stack.md) §10 |
...
Igual que con [14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md): **no se fusionan, se referencian.** La
...
**La diferencia con el caso de [14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md) es que acá no hubo
