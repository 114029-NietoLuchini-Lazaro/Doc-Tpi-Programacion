...
- **Dato clave:** es el **nombre histórico** del mismo servicio que hoy es `llm-service`
  ([02 · línea 3](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)) — ver
...
- **`AiController`**: `POST /ai/tutor`, recibe `AiRequest<PayloadTutorDto>`
  ...
  contrato de [02 · Parte 3, §1](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) (`POST /ai/{funcion}`).
...
- `prompts/{correccion,evaluacion,generacion,moderacion,tutor}/`: solo `tutor/` tiene contenido
  ...
  [04 · funciones de IA](../../../03-capacidades-de-ia/02-funciones-de-ia.md).
...
- `CorsConfig` con `allowedOriginPatterns("*")` — abierto para desarrollo; contradice
  [02 · §6](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) ("no se expone a internet").
...
- El patrón `AiRequest<T>`/`AiController` (`POST /ai/{funcion}`) como referencia del contrato
  genérico de [02 · Parte 3](../../../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md).
