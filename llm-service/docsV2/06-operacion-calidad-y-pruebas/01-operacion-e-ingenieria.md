...
> **Bus acordado.** Kafka es el bus compartido para eventos de integración. La cola de workers de `llm-service` sigue siendo interna y no se confunde con el bus. Los schemas y tópicos vigentes están en [`contracts/llm-service-v1.asyncapi.yaml`](../contracts/historicos-y-contratos-v1/llm-service-v1.asyncapi.yaml)
...
registrada como ADR-002 en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)

## 3. Por qué SÍ un gateway determinístico

Ya está desarrollado en [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) §3. El resumen: son ocho responsabilidades que
...
> vigente por función está en [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §1.
...
parciales al mismo tiempo?"*. Ver [06](01-operacion-e-ingenieria.md), pero el resumen es: **no pasa
...
barato, controlable y mejora mucho la calidad. Detalle en [04](../03-capacidades-de-ia/02-funciones-de-ia.md)
...
**Es una decisión del Product Owner, no tuya.** Está anotada en [08](../00-gobierno-y-evolucion/02-decisiones-y-pendientes.md)
...
| 5 | **Retirada de un modelo** | **Alta** — hay dos con fecha anunciada | Medio | Modelo en tabla de ADMIN, no en código. Ver [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §2 |
...
[03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §5.
...
| Qué | Por qué ayuda |
|---|---|
| **Guardar los features determinísticos** de cada transcripción (mensajes, tiempos, ediciones, incidentes) | Se calculan una vez y sirven para la evaluación, para la auditoría y para las apelaciones. Ver [04](../03-capacidades-de-ia/02-funciones-de-ia.md) §1b |
...
> Esta es la fuente normativa de los mensajes comunes de UX. La vista contractual de Front End
> conserva su mapeo de pantallas en [`contracts/equipos/frontend-angular.md`](../contracts/equipos/frontend-angular.md)
> y referencia esta sección; no hay una segunda tabla editable.
...
> Esta sección es la fuente normativa del componente y sus siete estados. El contrato de UI de
> Front End la referencia para indicar qué debe implementar el consumidor, sin copiar sus reglas
> técnicas.
...
| **Modo examen** (RF-EXA-01) | Fase 3, pero su concurrencia ya está dimensionada en [06](01-operacion-e-ingenieria.md)
...
En [12](../03-capacidades-de-ia/rag-e-ingesta/01-almacenamiento-e-ingesta.md) recomendé Redis para cola, cuotas y caché. **Revisando el
...
> La [U1 de Front End](../02-arquitectura-y-plataforma/02-sincronizacion-arquitectura-y-despliegue.md) dedica media unidad a esto y
...
[02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) —*un cambio incompatible sin aviso rompe al otro equipo*— en un
...
> ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) §3.3) y RF-IA-13 prohíbe recalcular puntajes históricos. Si una
...
| **producción** | Cursos reales | Los del catálogo de [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md)
