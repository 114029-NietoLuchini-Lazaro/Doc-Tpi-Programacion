...
> **Plan de ejecución:** [38 — Plan de 5 sprints](../07-planificacion-y-trabajo-equipo/05-plan-de-cinco-sprints.md) distribuye estos
> requisitos con un techo duro de 5 sprints ([23](../07-planificacion-y-trabajo-equipo/03-plan-de-construccion-del-producto.md) llegó a
...
|---|---|---|---|---|
| RF-IA-01/02/04/06/07/19/20 — tutor seguro y registro | MVP | OpenAPI: `POST /tutor/interactions`; adenda SSE [`contracts/llm-service-v1-tutor-sse-adenda.md`](../contracts/historicos-y-contratos-v1/llm-service-v1-tutor-sse-adenda.md). |
| Spike `LLM-S03-SPIKE-01` — viabilidad del streaming del tutor (EP-02) | MVP · S3 | Callout en [`35` · S3](../07-planificacion-y-trabajo-equipo/04-backlog-ejecutable.md); adenda SSE; nota en [17 · §8 · I-10](../contracts/90-mapa-de-integracion-historico.md) | Paquete 1 de S3 (puerto AI Gateway + `langchain4j`); solapa I-03 e I-11 | PoC medida —primer token, respuesta completa, costo de regeneración— y recomendación *propagar o revertir* que cierra I-10 y habilita, o no, fusionar la adenda al `openapi.yaml`. |
| RF-IA-12 a 18/25 — evaluador, detalle y apelación | MVP | OpenAPI: evaluaciones, apelaciones y overrides | `practice-service` publica `intento_cerrado.v1` (desde el 2026-09-13; antes `challenges-service`) | Cinco dimensiones, confianza, justificación, auditoría append-only y nunca XP. |
| RF-IA-30 a 36 — Golden Set y calibración | MVP | [32](../03-capacidades-de-ia/golden-set-y-calibracion/02-especificacion-funcional.md); OpenAPI: golden sets y calibrations; AsyncAPI | Docentes, `courses-service`, `admin-service` | Versiones publicadas inmutables; solo una calibración aprobada activa por curso; PAR-14 valida MAE final ≤ 5 y máximo caso/dimensión ≤ 10. |
| ADR-017 — rúbrica por curso | MVP | [08](02-decisiones-y-pendientes.md#adr-017--rúbricas-editables-y-versionadas-por-curso) | Docente, `courses-service` | Cinco dimensiones obligatorias; borrador editable; pesos suman 100 %; publicación inmutable. |
| ADR-018 — modelo por curso | MVP | [08](02-decisiones-y-pendientes.md#adr-018--selección-de-modelo-por-curso-sujeta-a-doble-calibración) | ADMIN y docente | El docente solo elige modelos habilitados que aprobaron calibración base y del curso. |
| Asociación desafío-calibración | MVP | [32 §11](../03-capacidades-de-ia/golden-set-y-calibracion/02-especificacion-funcional.md#11-activación-y-asociación-con-desafíos) | `challenges-service`, `courses-service` | Primer intento bloquea la versión; una activación solo migra desafíos con cero intentos y confirmación docente. |
| Validación académica del desafío | MVP, fuera de Tema 07 | [00](01-fuentes-de-verdad-y-convenciones.md)
| RF-IA-27/34 — cálculo diferido | MVP | AsyncAPI y consulta de pendientes | `practice-service` (desde el 2026-09-13; antes `challenges-service`), `courses-service` | Entrega aceptada ante caída; evento diferido; cierre bloqueado mientras existan pendientes. |
| RF-IA-22/23/24/35 — cuota y modelos | MVP | OpenAPI: model assignments | `admin-service` | 429 con `Retry-After`; cambio de modelo auditado sin cambio de código. Techo global por función; extensión propuesta a un techo por alumno y por día (cantidad de usos y tokens), configurable desde `admin-service` — [08](02-decisiones-y-pendientes.md#adr-017--rúbricas-editables-y-versionadas-por-curso)
| RF-NFR-01/09/10 — retención y auditoría | MVP | [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) y contratos v1 | T&C y política de plataforma | Sin hard delete académico; auditoría y retención verificables. |
| RF-NFR-03/04 — 120 sesiones y fallas | MVP | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) | Gateway, Kafka y proveedores | Carga concurrente y degradación sin bloquear entregas. |
...
| RF-IA-08 y RF-DES-05 — RAG e IA personalizada | Fase 3 | [04](../03-capacidades-de-ia/02-funciones-de-ia.md), roadmap | Cursos y docentes | No se implementa ni expone endpoint durante MVP. |
| Corrector LLM / parciales generados | Fuera del alcance vigente | Decisión P-01 y [00](01-fuentes-de-verdad-y-convenciones.md) | Product Owner | No existen endpoints, historias ni calibraciones de corrector; incorporarlos exige cambio de alcance y requisitos aprobados. |
...
[plantilla de sprint](../07-planificacion-y-trabajo-equipo/10-plantillas/sprint-llm.md)
