...
Este informe identifica solapamientos entre documentos de `docsV2` y deja constancia de las
...
| `contracts/90-mapa-de-integracion-historico.md` y contratos de equipos | Comparten actores, rutas y dependencias. | Mantener el mapa histórico y cada contrato de equipo; el detalle vigente por contraparte está centralizado en `docsV2/contracts/equipos/` y el mapa solo referencia esos documentos. |
...
|---|---|---|---|
| Alcance y límites del MVP | `00/01-fuentes...`, `01/01-problema...`, `01/02-entregables...`, `03/02-funciones...`, `06/07-control...` | `docsV2/01-alcance-mvp.md` + `docsV2/00-gobierno-y-fuentes-de-verdad.md` | Revisar antes de consolidar: evaluación, apelación y bloqueo tienen alcance que debe confirmar PO. |
| Identidad, Gateway, rutas y headers | `00/01-fuentes...`, `02/01-arquitectura...`, `contracts/90...`, `contracts/91...`, carpetas Gateway y contratos de equipos | `docsV2/contracts/` y reglas de Gateway de V2 | Consolidar después de verificar contra la arquitectura de plataforma. |
| Rúbrica, cinco dimensiones, pesos y publicación | `00/02-decisiones...` ADR-017, `00/03-glosario...`, `03/03-rubricas...`, `03/golden-set/02-especificacion...`, `03-dominio...` | `docsV2/03-dominio-y-flujos-mvp.md` + OpenAPI V2 | La regla de rúbricas editables está aprobada; retirar solo contradicciones antiguas de pesos fijos. |
...
| Anti-fuga y solución esperada | `00/01-fuentes...`, `00/02-decisiones...` ADR-008, `03/02-funciones...`, `04/01-seguridad...`, contratos Practice | `docsV2/04-seguridad-y-datos-sensibles.md` + `contracts/requisitos-a-otros-micros.md` | Pendiente del contrato real de Practice; no eliminar ningún detalle antes de cerrarlo. |
| Sincronía, colas, outbox, reintentos y resiliencia | `02/01-arquitectura...`, `06/01-operacion...`, `04/03-rate-limit...`, `07/06-playbook...`, Gateway | `docsV2/05-operacion-y-pruebas.md` y ADRs vigentes | Consolidar solo reglas técnicas; conservar ejemplos y decisiones históricas. |
...
| Diagramas de secuencia copiados en el mapa histórico y contratos | `contracts/equipos/tema-05-desafios-practicos.md` y `contracts/equipos/tema-11-chat.md` | `90-mapa-de-integracion-historico.md` conserva la perspectiva transversal y la decisión histórica I-04, y referencia los detalles. | No se perdió la decisión histórica ni el contexto de comparación; el flujo vigente tiene una sola descripción detallada por equipo. |
| Schema JSON del evaluador repetido en el contrato T05 | `contracts/llm-service.asyncapi.yaml`, mensaje `ScoreCalculated` | T05 enlaza al schema; `91-contratos-inter-equipos-historicos.md` conserva únicamente el ejemplo histórico `snake_case` y su contexto. | El contrato ejecutable define los campos una sola vez; la evidencia histórica sigue identificada como tal. |
...
- No hay archivos Markdown idénticos por contenido dentro de V2: la comparación SHA-256 dio **0
  grupos duplicados**.
