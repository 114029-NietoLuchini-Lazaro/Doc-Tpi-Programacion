...
documento V2 debe actualizarse cuando una regla cambie.
...
| Vigente con ajuste V2 | La regla se conserva, pero su nombre, alcance o contrato está normalizado en V2. |
...
| Regla o bloque | Estado | Fuente de consulta / destino V2 |
|---|---|---|
| Precedencia: producto aprobado, arquitectura de plataforma, contratos/ADR y resto de documentación | Vigente con ajuste V2 | [Fuentes y convenciones](01-fuentes-de-verdad-y-convenciones.md) §1 y [00 V2](../00-gobierno-y-fuentes-de-verdad.md). |
| Identidad `llm-service`, prefijo `/api/llm/**`, Gateway, M2M, headers y Problem Details | Vigente con ajuste V2 | [Fuentes y convenciones](01-fuentes-de-verdad-y-convenciones.md) §§2–4; contratos V2. |
| MVP tutor, anti-fuga, rúbrica, Golden Set, calibración, evaluación diferida y bloqueo | Vigente como alcance objetivo; implementación no inferida | [01 V2](../01-alcance-mvp.md) y [estado](../06-trazabilidad-y-estado.md). |
| Chat/moderación y RAG/`@mención` por fases posteriores | Futura / fuera de MVP | [Fuentes y convenciones](01-fuentes-de-verdad-y-convenciones.md) §5 y [01 V2](../01-alcance-mvp.md). |
| Cinco dimensiones, versiones inmutables publicadas y calibración por cohorte | Vigente con ajuste V2 | [03 V2](../03-dominio-y-flujos-mvp.md). La edición en borrador está regida por ADR-017. |
| Intercambio indirecto con `challenges-service` a través de `practice-service` | Vigente con ajuste V2 | [Fuentes y convenciones](01-fuentes-de-verdad-y-convenciones.md) §6 y [mapa V2](../contracts/00-mapa-de-integracion.md). |

## ADR del documento 08 — decisiones y pendientes

| ADR | Estado | Regla aplicable en V2 |
|---|---|---|
| ADR-001, gateway único de IA | Vigente con ajuste V2 | El servicio se denomina `llm-service`; ningún frontend o micro de negocio llama al proveedor. |
| ADR-002, ruteo determinístico | Vigente | La operación/endpoint determina la función; no hay orquestador LLM. |
| ADR-003, sincronía y trabajos diferidos | Vigente con ajuste V2 | Tutor síncrono; calibración y evaluación diferida asíncronas. Corrector y moderador no pertenecen al MVP. |
...
| ADR-005, Java/Spring Boot | Vigente con ajuste V2 | Java y `llm-service`; el nombre histórico `ms-evaluacion-llm` no es canónico. |
...
| ADR-009, streaming | Reemplazada | V2 mantiene la regla MVP de no streaming token a token. La propuesta posterior de Buffer Interceptor quedó como antecedente hasta decisión explícita. |
...
| ADR-013, rolling update; ADR-014, readiness sin proveedor; ADR-015, nginx en el borde | Vigente con ajuste V2 | Operación y plataforma; normalizar nombres/rutas al tocar sus documentos. |
...
|---|---|---|
| [Glosario y metadata](03-glosario-y-metadata.md) | Vigente con ajuste V2 | Usar términos no contradictorios; nombres de tablas/campos siguen siendo propuesta hasta acordarlos. RAG y moderación conservan estado de fase futura. |
| [Matriz de trazabilidad](04-matriz-trazabilidad.md) | Histórica | Evidencia de preparación; el estado actual se declara en [06 V2](../06-trazabilidad-y-estado.md). |
| [Informe comparativo](05-informe-de-alineacion.md) | Histórica | Explica una alineación anterior; contrastar siempre con V2 y contratos actuales. |
...
| [Control de consolidación](07-control-de-consolidacion-y-entrega.md) | Histórica con acciones pendientes | Sus brechas de contrato se trasladaron a `contracts/`; no usar OpenAPI V1/V2 históricos como fuente única. |
| [Informe de auditoría](08-auditoria-de-convenciones.md) | Histórica | Auditoría de una presentación. Las correcciones de nombre/ruteo siguen vigentes, pero rutas funcionales se toman de OpenAPI V2. |

## Regla de mantenimiento

Al resolver una fila marcada **Pendiente** o cambiar una regla **Vigente**, se actualiza primero el
documento V2 canónico, después esta matriz y finalmente se agrega una entrada con antes, motivo,
decisión y después en [registro](../registro/README.md). El antecedente V1 no se reescribe ni se
elimina.
