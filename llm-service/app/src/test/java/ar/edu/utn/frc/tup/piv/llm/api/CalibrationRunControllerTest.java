...
class CalibrationRunControllerTest {
  ...
  void authorizesTeacherAndListsCalibrationRuns() {
    ...
    var controller = new CalibrationRunController(service, auth, courses, mock(ProviderCredentialRepository.class));
  ...
  void authorizesTeacherAndCreatesCalibrationRun() {
    ...
    var controller = new CalibrationRunController(service, auth, courses, deployments);
    ...
    CallerIdentity actor = new CallerIdentity("workbench", UUID.randomUUID(), null, null);
    when(auth.require(headers)).thenReturn(actor);
    when(deployments.calibrationTarget()).thenReturn(java.util.Optional.of(
        new ProviderCredentialRepository.Deployment(modelId, UUID.randomUUID(), "openai-compatible",
            "OpenAI de prueba", "gpt-4o-mini", "CANDIDATE", java.time.Instant.now(),
            1, java.time.Instant.now(), java.util.Map.of())));
