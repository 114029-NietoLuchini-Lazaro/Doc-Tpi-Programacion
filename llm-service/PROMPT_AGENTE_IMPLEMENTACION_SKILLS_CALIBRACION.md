...
1. `llm-service/AGENTS.md`: reglas de arquitectura, seguridad, persistencia,
   contratos y calidad.
2. `llm-service/Plan_skills_calibracion.md`: fuente de decisiones de Producto.
   Es un registro incremental; distinguí decisiones confirmadas de propuestas.
3. `llm-service/docsV2/03-capacidades-de-ia/golden-set-y-calibracion/`:
   especificación funcional y modelo vigente.
4. `llm-service/docsV2/contracts/README.md`,
   `00-mapa-de-integracion.md`, `01-inventario-y-brechas.md` y
   `requisitos-a-otros-micros.md`.
5. `llm-service/docsV2/contracts/llm-service.openapi.yaml` y
   `llm-service/docsV2/contracts/llm-service.asyncapi.yaml`: son los schemas
   ejecutables actuales, no los sustituyas por texto narrativo.
6. `llm-service/docsV2/contracts/02-contratos-skills-y-calibracion.md`:
...
`Plan_skills_calibracion.md` y recién después ejecutá su implementación siguiendo
